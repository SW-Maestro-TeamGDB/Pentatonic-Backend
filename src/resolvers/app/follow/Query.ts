...
export const getFollowerList = async (
    parent: void,
    args: GetFollowerListInput,
    context: Context
) => {
    const userIds = await context.db
        .collection("follow")
        .find({
...
export const getRankedUser = async (
    parent: void,
    args: void,
    context: Context
) => {
    const followCounts = await context.db
        .collection("follow")
        .aggregate([
            {
                $group: {
